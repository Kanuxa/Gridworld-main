...
from models.shared.ppo_rnd_interface import (
    ...
    choose_device,
    init_runtime_context as base_init_runtime_context,
    obs_to_state,
    onehot_action,
    reset_runtime_context as base_reset_runtime_context,
    update_runtime_context_after_env_step as base_update_runtime_context_after_env_step,
)
...
TRAINER_GUI_INTERFACE_VERSION = "symmetric-recurrent-ppo-rnd"
TRAINER_DISPLAY_NAME = "Symmetric recurrent PPO/RND with frontier prior"
...
class TrainConfig:
    updates: int = 300
    seed: int = 7
    save_dir: str = "runs/symmetric_recurrent_ppo_rnd"
    device: str = "auto"
...
def swap_left_right_np(arr: np.ndarray) -> np.ndarray:
    out = np.array(arr, copy=True)
    out[..., ACTION_LEFT] = arr[..., ACTION_RIGHT]
    out[..., ACTION_RIGHT] = arr[..., ACTION_LEFT]
    return out


def swap_left_right_torch(tensor: torch.Tensor) -> torch.Tensor:
    out = tensor.clone()
    out[..., ACTION_LEFT] = tensor[..., ACTION_RIGHT]
    out[..., ACTION_RIGHT] = tensor[..., ACTION_LEFT]
    return out


def mirror_state_dict_np(state: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
    out: Dict[str, np.ndarray] = {}
    for key, value in state.items():
        if key == "scalars":
            out[key] = value.copy()
        else:
            out[key] = np.flip(value, axis=-1).copy()
    return out


def mirror_state_batch_np(state_batch: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
    out: Dict[str, np.ndarray] = {}
...
def mirror_state_sequence_torch(state: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    out: Dict[str, torch.Tensor] = {}
    for key, value in state.items():
        if key == "scalars":
            out[key] = value
        else:
            out[key] = torch.flip(value, dims=[-1])
    return out


def symmetrized_forward_sequence(
    net: RecurrentPPORNDNetwork,
    state: Dict[str, torch.Tensor],
    prev_action: torch.Tensor,
    prev_reward: torch.Tensor,
    hidden: torch.Tensor,
    hidden_mirror: torch.Tensor,
    reset_mask: torch.Tensor | None = None,
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    logits, ext_value, int_value, hidden_out = net.forward_sequence(
        state,
        prev_action,
        prev_reward,
        hidden,
    ...
    mirror_logits, mirror_ext_value, mirror_int_value, hidden_mirror_out = net.forward_sequence(
        mirror_state_sequence_torch(state),
        swap_left_right_torch(prev_action),
        prev_reward,
        hidden_mirror,
        reset_mask=reset_mask,
    )
    sym_logits = 0.5 * (logits + swap_left_right_torch(mirror_logits))
    sym_ext_value = 0.5 * (ext_value + mirror_ext_value)
    sym_int_value = 0.5 * (int_value + mirror_int_value)
    return sym_logits, sym_ext_value, sym_int_value, hidden_out, hidden_mirror_out
...
def rollout_batch_to_training_format(batch: Dict[str, Any], cfg: TrainConfig) -> Dict[str, np.ndarray]:
    ...
    return {
        ...
        "hidden_start_mirror": batch["hidden_start_mirror"].astype(np.float32),
    }
...
def collect_rollout(
    ...
    episodes_seen: int,
    global_step: int,
) -> Tuple[Dict[str, Any], List[Dict[str, Any]], List[Dict[str, np.ndarray]], np.ndarray, np.ndarray, torch.Tensor, torch.Tensor, np.ndarray, List[Dict[str, object]], np.ndarray, int, int, int]:
    ...
    hidden_start_mirror = hidden_mirror.detach().cpu().numpy()
    ...
    for t in range(rollout_steps):
        ...
        with torch.no_grad():
            logits, ext_value, int_value, hidden, hidden_mirror = symmetrized_forward_sequence(
                net,
                state_batch,
                prev_action_t,
                prev_reward_t,
    ...
    with torch.no_grad():
        _, last_ext_value, last_int_value, _, _ = symmetrized_forward_sequence(
            net,
            last_state_t,
            last_prev_action_t,
            last_prev_reward_t,
    ...
    batch = {
        ...
        "hidden_start_mirror": hidden_start_mirror,
...
def ppo_update(
    ...
    hidden_start_mirror_t = torch.from_numpy(batch["hidden_start_mirror"]).to(device)
    ...
    for _ in range(int(cfg.ppo_epochs)):
        ...
        for start in range(0, n_envs, minibatch_envs):
            ...
            logits, ext_values, int_values, _, _ = symmetrized_forward_sequence(
                net,
                {k: v[idx] for k, v in states_t.items()},
                prev_actions_t[idx],
                prev_rewards_t[idx],
                hidden_start_t[:, idx],
                hidden_start_mirror_t[:, idx],
...
def evaluate_policy(
    ...
    for ep in range(episodes):
        ...
        hidden_mirror = net.initial_hidden(1, device)
        ...
        while not done:
            ...
            logits, _, _, hidden, hidden_mirror = symmetrized_forward_sequence(
                net,
                state_t,
                prev_action_t,
                prev_reward_t,
...
def init_runtime_context(device: str = "cpu") -> Dict[str, Any]:
    ctx = base_init_runtime_context(device=device)
    ctx["hidden_mirror"] = None
    return ctx


def reset_runtime_context(context: Dict[str, Any] | None = None, device: str | None = None) -> Dict[str, Any]:
    ctx = base_reset_runtime_context(context, device=device)
    ctx["hidden_mirror"] = None
    return ctx
...
def update_runtime_context_after_env_step(
    ...
    updated = base_update_runtime_context_after_env_step(ctx, action, reward, done=False)
    updated["hidden_mirror"] = ctx.get("hidden_mirror", None)
    return updated
...
def predict_action_for_gui(
    ...
    hidden_mirror = ctx.get("hidden_mirror", None)
    if hidden is None:
        hidden = net.initial_hidden(1, device)
    if hidden_mirror is None:
        hidden_mirror = net.initial_hidden(1, device)
    with torch.no_grad():
        logits, _, _, hidden_out, hidden_mirror_out = symmetrized_forward_sequence(
            net,
            state_t,
            prev_action_t,
            prev_reward_t,
    ...
    next_ctx["hidden_mirror"] = hidden_mirror_out.detach()
...
def checkpoint_payload(
    ...
    return {
        ...
        "model_arch": "patch_fusion_gru_ppo_rnd_symmetric_frontier_prior",
        "model_kwargs": {
            "patch_size": env.config.patch_size,
            "num_actions": N_ACTIONS,
            "use_vision": switches.include_vision,
            ...
            "algorithm": "ppo_rnd_symmetric_frontier_prior",
        },
        "input_dim": input_dim,
        "num_actions": N_ACTIONS,
...
def write_train_config(save_dir: Path, cfg: TrainConfig, env: SensoryGridEnv, switches: ObservationSwitches, input_dim: int) -> None:
    with open(save_dir / "train_config.txt", "w", encoding="utf-8") as f:
        f.write("model_arch: patch_fusion_gru_ppo_rnd_symmetric_frontier_prior\n")
...
def get_gui_interface_spec() -> Dict[str, Any]:
    return {
        ...
        "model_family": "patch_fusion_gru_ppo_rnd_symmetric_frontier_prior",
        "default_switches": asdict(build_default_switches()),
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Symmetric recurrent PPO/RND with frontier prior")
    parser.add_argument("--updates", type=int, default=300)
    parser.add_argument("--save_dir", type=str, default="runs/symmetric_recurrent_ppo_rnd")
    ...
    hidden_mirror = net.initial_hidden(cfg.n_envs, device)
    ...
    for update in range(1, cfg.updates + 1):
        rollout_batch, current_obs, current_states, prev_actions, prev_rewards, hidden, hidden_mirror, episode_starts, diag_trackers, episode_returns, reset_seed_cursor, episodes_seen, global_step = collect_rollout(
            envs,
            switches,
            net,
            device,
