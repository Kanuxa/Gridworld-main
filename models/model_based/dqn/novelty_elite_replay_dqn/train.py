...
TRAINER_GUI_INTERFACE_VERSION = "novelty-elite-replay-dqn"
TRAINER_DISPLAY_NAME = "Novelty and elite-replay recurrent Double DQN"
...
class TrainConfig:
    ...
    save_dir: str = "runs/novelty_elite_replay_dqn"
    device: str = "auto"
    ...
    # v7 change 1: use episodic novelty over egocentric observations to
    # distinguish genuinely new local situations from repeated loops.
    use_observation_novelty_bonus: bool = True
    train_obs_novelty_bonus_scale: float = 0.015
    replay_obs_novelty_weight: float = 0.65

    # v7 change 2: maintain a replay pool of top-coverage episodes and apply a
    # light self-imitation margin loss so high-coverage trajectories are reused.
    use_elite_replay: bool = True
    elite_replay_capacity_episodes: int = 256
    elite_priority_alpha: float = 0.50
    elite_min_episodes: int = 40
    elite_train_updates_per_episode: int = 4
    elite_imitation_loss_weight: float = 0.10
    elite_imitation_margin: float = 0.05
...
class EpisodeSequenceReplayBuffer:
    ...
    def sample(self, batch_size: int, total_len: int) -> Dict[str, object]:
        ...
        batch_imitation_weight: List[np.ndarray] = []
        ...
        for _ in range(batch_size):
            ...
            seq_imitation_weight = np.zeros((total_len,), dtype=np.float32)
            ...
            for j in range(total_len):
                ...
                if idx < len(ep):
                    ...
                    seq_imitation_weight[j] = float(tr.get("imitation_weight", 0.0))
            ...
            batch_imitation_weight.append(seq_imitation_weight)
        ...
        return {
            ...
            "imitation_weight": np.stack(batch_imitation_weight).astype(np.float32),
            "revisit": np.stack(batch_revisit).astype(np.float32),
            "step_index": np.stack(batch_step_index).astype(np.float32),
        }


class EliteEpisodeReplayBuffer(EpisodeSequenceReplayBuffer):
    def __init__(self, capacity_episodes: int, priority_alpha: float = 0.50):
        super().__init__(capacity_episodes, priority_alpha)
        self.entries: List[Tuple[float, int, List[Dict[str, object]]]] = []
        self.insert_counter = 0

    def add_episode(self, transitions: List[Dict[str, object]], priority: float) -> None:
        if not transitions:
            return
        self.entries.append((float(max(priority, 1e-3)), self.insert_counter, transitions))
        self.insert_counter += 1
        self.entries.sort(key=lambda item: (item[0], item[1]), reverse=True)
        if len(self.entries) > self.capacity_episodes:
            self.entries = self.entries[: self.capacity_episodes]
        self.episodes = deque((entry[2] for entry in self.entries), maxlen=self.capacity_episodes)
        self.priorities = deque((entry[0] for entry in self.entries), maxlen=self.capacity_episodes)
...
def train_step(
    ...
    imitation_weight_t = torch.from_numpy(batch["imitation_weight"]).to(device)
    ...
    imitation_weight_main = imitation_weight_t[:, cfg.burn_in:]
    ...
    if cfg.use_exploration_reward_shaping:
        ...
        shaped_rewards_main = rewards_main + coverage_bonus + novelty_bonus - energy_penalty - revisit_penalty
    ...
    if imitation_loss_weight > 0.0:
        imitation_mask = mask & (imitation_weight_main > 1e-6)
        if bool(imitation_mask.any().item()):
            action_q = chosen_q[imitation_mask]
            other_q = q_seq.masked_fill(
                nn.functional.one_hot(actions_main, num_classes=N_ACTIONS).bool(),
                float("-inf"),
            ).amax(dim=-1)[imitation_mask]
            weights = imitation_weight_main[imitation_mask]
            margin = float(cfg.elite_imitation_margin)
            imitation_loss = (torch.relu(other_q + margin - action_q) * weights).sum() / weights.sum().clamp_min(1.0)
        else:
            imitation_loss = torch.tensor(0.0, device=device)
    else:
        imitation_loss = torch.tensor(0.0, device=device)

    total_loss = q_loss + aux_weight * aux_loss + imitation_loss_weight * imitation_loss
...
def checkpoint_payload(
    ...
    return {
        ...
        "model_arch": "patch_fusion_gru_dueling_double_dqn_novelty_elite_replay",
        "model_kwargs": {
            "patch_size": env.config.patch_size,
            "num_actions": N_ACTIONS,
            "use_vision": switches.include_vision,
...
def write_train_config(save_dir: Path, cfg: TrainConfig, env: SensoryGridEnv, switches: ObservationSwitches, input_dim: int) -> None:
    with open(save_dir / "train_config.txt", "w", encoding="utf-8") as f:
        f.write("model_arch: patch_fusion_gru_dueling_double_dqn_novelty_elite_replay\n")
...
def episode_priority(last_info: Dict[str, object], ep_reward: float, diag_metrics: Dict[str, float]) -> float:
    ...
    coverage_gain_per_energy = float(diag_metrics.get("coverage_gain_per_energy", 0.0))
    new_tiles_per_energy = float(diag_metrics.get("new_tiles_per_energy", 0.0))
    obs_novelty_mean = float(diag_metrics.get("obs_novelty_mean", 0.0))
    ...
    return (
        ...
        + 0.30 * new_tiles_per_energy
        + 0.10 * obs_novelty_mean
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Novelty and elite-replay recurrent Double DQN")
    parser.add_argument("--episodes", type=int, default=1200)
    parser.add_argument("--save_dir", type=str, default="runs/novelty_elite_replay_dqn")
    ...
    parser.add_argument("--disable_elite_replay", action="store_true")
    ...
    cfg = TrainConfig(
        ...
        use_elite_replay=not args.disable_elite_replay,
    ...
    elite_replay = EliteEpisodeReplayBuffer(cfg.elite_replay_capacity_episodes, cfg.elite_priority_alpha)
    ...
    for episode in range(1, cfg.episodes + 1):
        ...
        while not done:
            ...
            episode_diag["obs_novelty_total"] = float(episode_diag["obs_novelty_total"]) + float(obs_novelty)
        ...
        final_coverage = float(last_info.get("coverage", current_coverage))
        coverage_span = max(final_coverage - float(reset_info.get("coverage", 0.0)), 1e-6)
        for transition in episode_transitions:
            future_gain = max(0.0, final_coverage - float(transition.get("coverage_before", 0.0)))
            transition["imitation_weight"] = float(future_gain / coverage_span)
        ep_priority = episode_priority(last_info, ep_reward, episode_diag_metrics)
        replay.add_episode(episode_transitions, priority=ep_priority)
        if cfg.use_elite_replay:
            elite_replay.add_episode(episode_transitions, priority=ep_priority)
        ...
        if len(replay) >= cfg.train_after_episodes:
            for _ in range(cfg.train_updates_per_episode):
                total_loss, q_loss, aux_loss, imitation_loss = train_step(
                    online_net,
                    target_net,
                    optimizer,
                    replay,
            ...
            if (
                cfg.use_elite_replay
                and episode >= cfg.elite_min_episodes
                and len(elite_replay) >= max(8, cfg.batch_size // 4)
            ):
                for _ in range(cfg.elite_train_updates_per_episode):
                    total_loss, q_loss, aux_loss, imitation_loss = train_step(
                        online_net,
                        target_net,
                        optimizer,
                        ...
                        cfg,
                        aux_weight,
                        imitation_loss_weight=cfg.elite_imitation_loss_weight,
                    )
                    losses.append(total_loss)
                    q_losses.append(q_loss)
                    aux_losses.append(aux_loss)
        ...
        print(
            ...
            f"loss={mean_loss:.4f} q={mean_q_loss:.4f} aux={mean_aux_loss:.4f} ilm={mean_imitation_loss:.4f} aw={aux_weight:.3f} "
