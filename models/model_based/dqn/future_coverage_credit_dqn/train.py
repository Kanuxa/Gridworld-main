...
TRAINER_GUI_INTERFACE_VERSION = "future-coverage-credit-dqn"
TRAINER_DISPLAY_NAME = "Future-coverage-credit recurrent Double DQN"
...
class TrainConfig:
    ...
    save_dir: str = "runs/future_coverage_credit_dqn"
    device: str = "auto"
...
def train_step(
    ...
    if cfg.use_exploration_reward_shaping:
        ...
        future_bonus = float(cfg.train_future_coverage_bonus_scale) * future_coverage_gain_main * (
        ...
        shaped_rewards_main = rewards_main + coverage_bonus + future_bonus + novelty_bonus - energy_penalty - revisit_penalty
...
def checkpoint_payload(
    ...
    return {
        ...
        "model_arch": "patch_fusion_gru_dueling_double_dqn_future_coverage_credit",
        "model_kwargs": {
            "patch_size": env.config.patch_size,
            "num_actions": N_ACTIONS,
            "use_vision": switches.include_vision,
...
def write_train_config(save_dir: Path, cfg: TrainConfig, env: SensoryGridEnv, switches: ObservationSwitches, input_dim: int) -> None:
    with open(save_dir / "train_config.txt", "w", encoding="utf-8") as f:
        f.write("model_arch: patch_fusion_gru_dueling_double_dqn_future_coverage_credit\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Future-coverage-credit recurrent Double DQN")
    parser.add_argument("--episodes", type=int, default=1200)
    parser.add_argument("--save_dir", type=str, default="runs/future_coverage_credit_dqn")
