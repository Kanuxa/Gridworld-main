...
TRAINER_GUI_INTERFACE_VERSION = "recurrent-patch-fusion-initial-heuristics"
TRAINER_DISPLAY_NAME = "Recurrent patch-fusion DQN with initial exploration heuristics"
...
class TrainConfig:
    ...
    save_dir: str = "runs/recurrent_patch_fusion_initial_heuristics"
    device: str = "auto"
    ...
    exploration_frontier_score_scale: float = 0.90
...
def checkpoint_payload(
    ...
    return {
        ...
        "model_arch": "patch_fusion_gru_dueling_double_dqn_initial_exploration_heuristics",
        "model_kwargs": {
            "patch_size": env.config.patch_size,
            "num_actions": N_ACTIONS,
            "use_vision": switches.include_vision,
...
def write_train_config(save_dir: Path, cfg: TrainConfig, env: SensoryGridEnv, switches: ObservationSwitches, input_dim: int) -> None:
    with open(save_dir / "train_config.txt", "w", encoding="utf-8") as f:
        f.write("model_arch: patch_fusion_gru_dueling_double_dqn_initial_exploration_heuristics\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Recurrent patch-fusion Double DQN with initial exploration heuristics")
    parser.add_argument("--episodes", type=int, default=1200)
    parser.add_argument("--save_dir", type=str, default="runs/recurrent_patch_fusion_initial_heuristics")
