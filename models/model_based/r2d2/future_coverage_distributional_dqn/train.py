...
TRAINER_GUI_INTERFACE_VERSION = "future-coverage-distributional-dqn"
TRAINER_DISPLAY_NAME = "Future-coverage recurrent distributional Double DQN"
...
class TrainConfig:
    ...
    save_dir: str = "runs/future_coverage_distributional_dqn"
    device: str = "auto"
    ...
    train_updates_per_episode: int = 8

    target_soft_tau: float = 0.05
    target_hard_sync_every_episodes: int = 80
    ...
    use_state_adaptive_exploration: bool = True
    ...
    train_obs_novelty_bonus_scale: float = 0.020
    train_future_coverage_horizon: int = 10
    train_future_coverage_scale: float = 2.50
    train_future_turn_credit_multiplier: float = 1.35
    replay_future_coverage_scale: float = 40.0
    ...
    exploration_frontier_score_scale: float = 0.12
    ...
    exploration_frontier_side_weight: float = 0.75
    exploration_frontier_turn_penalty: float = 0.05
    exploration_turn_recovery_trigger_steps: int = 5
    exploration_turn_recovery_ramp_steps: int = 8
    exploration_turn_recovery_mix: float = 0.30
    exploration_no_move_turn_mix: float = 0.18
    exploration_min_left_turn_prob: float = 0.08
    exploration_min_right_turn_prob: float = 0.10
    exploration_right_bias_if_collapsed: float = 0.20
...
def rebalance_structured_exploration_probs(
    probs: np.ndarray,
    obs: Dict[str, Any],
    ...
    action_counts = episode_diag.get("action_counts", {})
    if isinstance(action_counts, dict):
        left_count = int(action_counts.get(ACTION_LEFT, 0))
        right_count = int(action_counts.get(ACTION_RIGHT, 0))
    else:
        left_count = 0
        right_count = 0
    ...
    steps_since_new_tile = int(episode_diag.get("steps_since_new_tile", 0))
    no_move_steps = int(obs.get("consecutive_no_move_steps", 0))
    trigger = max(1, int(cfg.exploration_turn_recovery_trigger_steps))
    ramp = max(1, int(cfg.exploration_turn_recovery_ramp_steps))

    mix = 0.0
    if steps_since_new_tile >= trigger:
        mix += float(cfg.exploration_turn_recovery_mix) * float(
            np.clip((steps_since_new_tile - trigger + 1) / ramp, 0.0, 1.0)
        )
    if no_move_steps >= 2:
        mix += float(cfg.exploration_no_move_turn_mix) * float(np.clip((no_move_steps - 1) / 2.0, 0.0, 1.0))

    if mix > 0.0:
        if left_count >= max(4, 2 * max(1, right_count)):
            prior = np.array(
                [
                    1.0 - float(cfg.exploration_min_left_turn_prob) - float(cfg.exploration_right_bias_if_collapsed),
                    float(cfg.exploration_min_left_turn_prob),
                    float(cfg.exploration_right_bias_if_collapsed),
                ],
                dtype=np.float64,
            )
        elif right_count >= max(4, 2 * max(1, left_count)):
            prior = np.array(
                [
                    1.0 - float(cfg.exploration_min_right_turn_prob) - float(cfg.exploration_right_bias_if_collapsed),
                    float(cfg.exploration_right_bias_if_collapsed),
                    float(cfg.exploration_min_right_turn_prob),
                ],
                dtype=np.float64,
            )
        else:
            prior = np.array([0.40, 0.28, 0.32], dtype=np.float64)
        probs = (1.0 - min(mix, 0.45)) * probs + min(mix, 0.45) * prior

    if steps_since_new_tile >= trigger or left_count >= max(4, 2 * max(1, right_count)):
        probs[ACTION_LEFT] = max(probs[ACTION_LEFT], float(cfg.exploration_min_left_turn_prob))
        probs[ACTION_RIGHT] = max(probs[ACTION_RIGHT], float(cfg.exploration_min_right_turn_prob))
...
def checkpoint_payload(
    ...
    return {
        ...
        "model_arch": "patch_fusion_lstm_dueling_double_c51_future_coverage_dqn",
        "model_kwargs": {
            "patch_size": env.config.patch_size,
            "num_actions": N_ACTIONS,
            "use_vision": switches.include_vision,
...
def write_train_config(save_dir: Path, cfg: TrainConfig, env: SensoryGridEnv, switches: ObservationSwitches, input_dim: int) -> None:
    with open(save_dir / "train_config.txt", "w", encoding="utf-8") as handle:
        handle.write("model_arch: patch_fusion_lstm_dueling_double_c51_future_coverage_dqn\n")
    ...
    with torch.no_grad():
        ...
        if random.random() < epsilon:
            if cfg.use_structured_exploration:
                ...
                probs = rebalance_structured_exploration_probs(probs, obs, episode_diag, cfg)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Future-coverage recurrent distributional C51 Double DQN")
    parser.add_argument("--episodes", type=int, default=1200)
    parser.add_argument("--save_dir", type=str, default="runs/future_coverage_distributional_dqn")
    ...
    parser.add_argument("--burn_in", type=int, default=TrainConfig.burn_in)
    parser.add_argument("--unroll_len", type=int, default=TrainConfig.unroll_len)
    parser.add_argument("--train_updates_per_episode", type=int, default=TrainConfig.train_updates_per_episode)
    parser.add_argument("--replay_priority_alpha", type=float, default=TrainConfig.replay_priority_alpha)
    parser.add_argument("--v_min", type=float, default=TrainConfig.v_min)
    parser.add_argument("--v_max", type=float, default=TrainConfig.v_max)
    parser.add_argument("--future_coverage_horizon", type=int, default=TrainConfig.train_future_coverage_horizon)
    parser.add_argument("--future_coverage_scale", type=float, default=TrainConfig.train_future_coverage_scale)
    ...
    parser.add_argument("--disable_state_adaptive_exploration", action="store_true")
    parser.add_argument("--disable_episodic_exploration_bonus", action="store_true")
    args = parser.parse_args()

    use_state_adaptive_exploration = TrainConfig.use_state_adaptive_exploration
    if args.enable_state_adaptive_exploration:
        use_state_adaptive_exploration = True
    if args.disable_state_adaptive_exploration:
        use_state_adaptive_exploration = False
    ...
    cfg = TrainConfig(
        ...
        train_updates_per_episode=args.train_updates_per_episode,
        replay_priority_alpha=args.replay_priority_alpha,
        v_min=args.v_min,
        v_max=args.v_max,
        train_future_coverage_horizon=args.future_coverage_horizon,
        train_future_coverage_scale=args.future_coverage_scale,
        use_structured_exploration=not args.disable_structured_exploration,
        use_state_adaptive_exploration=use_state_adaptive_exploration,
    ...
    for episode in range(1, cfg.episodes + 1):
        ...
        while not done:
            ...
            action, hidden = choose_action(online_net, obs, state, prev_action, episode_diag, hidden, device, epsilon, cfg)
            next_obs, reward, terminated, truncated, info = env.step(action, switches)
            done = terminated or truncated
            ...
            transitions.append(
                {
                    ...
                    "coverage_after": float(next_coverage),
                }
            )
        ...
        future_horizon = max(1, int(cfg.train_future_coverage_horizon))
        for idx, transition in enumerate(transitions):
            immediate_coverage_gain = max(
                0.0,
                float(transition.get("coverage_after", 0.0)) - float(transition.get("coverage_before", 0.0)),
            )
            future_peak_coverage = float(transition.get("coverage_after", transition.get("coverage_before", 0.0)))
            for future_idx in range(idx + 1, min(len(transitions), idx + future_horizon)):
                future_peak_coverage = max(
                    future_peak_coverage,
                    float(transitions[future_idx].get("coverage_after", future_peak_coverage)),
                )
            future_extra_coverage_gain = max(
                0.0,
                future_peak_coverage - float(transition.get("coverage_before", 0.0)) - immediate_coverage_gain,
            )
            future_credit = float(cfg.train_future_coverage_scale) * future_extra_coverage_gain
            if int(transition["action"]) in (ACTION_LEFT, ACTION_RIGHT):
                future_credit *= float(cfg.train_future_turn_credit_multiplier)
            transition["future_coverage_gain"] = float(future_extra_coverage_gain)
            transition["train_reward"] = float(transition["train_reward"]) + future_credit
            transition["sample_weight"] = float(
                max(
                    0.05,
                    float(transition["sample_weight"]) + float(cfg.replay_future_coverage_scale) * future_extra_coverage_gain,
                )
            )
